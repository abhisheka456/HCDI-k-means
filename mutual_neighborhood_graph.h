...
void compute_mutual_knn(int n, int k,
    ...
    vector <int> m_hat(n);
    vector <int> cluster_membership(n);
...
void calculate_centre(int n,
                      ...
                      double * ns_centre){
int idx = 0;
//cout << ns_cluster << endl;
for (int i = 0; i < ns_cluster; i++) {
    for (int j = 0; j < n; j++) {
        if (membership[j] == i) {
            for (int k = 0; k < d; k++) {
                ns_centre[i*d+k] += X[j*d+k];
            }
            idx += 1;
        }
    ...
    //cout << idx << endl;
    for (int k = 0; k < d; k++) {
        ns_centre[i*d+k] = ns_centre[i*d+k]/idx;
       // cout << ns_centre[i*d+k] << endl;
    }
    idx = 0;
